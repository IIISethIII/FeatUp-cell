...
@hydra.main(config_path="configs", config_name="generalized_implicit_upsampler.yaml")
def my_app(cfg: DictConfig) -> None:
    ...
    upsampler = get_implicit_upsampler(
        start_dim=5 * cfg.n_freqs * 2 + 3 if cfg.color_feats else 2 * cfg.n_freqs * 2, 
        end_dim=cfg.proj_dim,
        color_feats=cfg.color_feats,
        n_freqs=cfg.n_freqs
    ).cuda()
    ...
    if cfg.outlier_detection:
        ...
        get_scale = lambda feats: torch.ones(feats.shape[0], 1, feats.shape[2], feats.shape[2], device=feats.device, dtype=feats.dtype)
    ...
    for epoch in range(cfg.epochs):
        for img_num, batch in enumerate(loader):
            ...
            jitter_loader = DataLoader(dataset, featurize_batch_size)
            ...
            with torch.no_grad():
                ...
                jit_features = []
                for transformed_image, tp in tqdm(jitter_loader):
                    for k, v in tp.items():
            ...
            for step in tqdm(range(steps), f"Epoch {epoch} Image {img_num} of {partition_size}"):
        ...
        torch.save({
            ...
            "downsampler_state_dict": downsampler.state_dict(),
            "unprojector_state_dict": unprojector.state_dict()
        }, join(feat_dir, "generalized_model.pth"))
